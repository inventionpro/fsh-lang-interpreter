function evaluate(expression) {
    ...
    function tokenize(exp) {
        ...
        for (let char of exp) {
            if (/\d/.test(char)) { // If the character is a digit, build a number
                numberBuffer += char;
            } else {
                if (numberBuffer.length > 0) {
                    tokens.push(Number(numberBuffer)); // Push the built number as token
                    numberBuffer = '';
                }
                if (/[+\-*/()]/.test(char)) {
                    tokens.push(char); // Push operators and parentheses as tokens
                }
            }
        }
        if (numberBuffer.length > 0) {
